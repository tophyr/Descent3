...
HWND InitDummy() {
  ...
  // Create Window
  if (!(dummy = CreateWindowEx(exStyle, TEXT("PiccuOpenGLDummyWindow"), TEXT("PiccuEngine"),
                               WS_CLIPSIBLINGS | WS_CLIPCHILDREN | style, 0, 0, windowRect.right - windowRect.left,
                               windowRect.bottom - windowRect.top, NULL, NULL, g_hInst, NULL))) {
...
void APIENTRY GL_LogDebugMsg(GLenum source, GLenum type, GLuint id, GLenum severity, GLsizei length, const GLchar *msg,
                             const void *user) {
