...
void GL_UseDrawVAO(void) { glBindVertexArray(drawvao); }
...
// as a texture
void opengl_DrawMultitexturePolygon3D(int handle, g3Point **p, int nv, int map_type) { Int3(); }

void opengl_DrawFlatPolygon3D(g3Point **p, int nv) { Int3(); }
...
void rend_DrawSpecialLine(g3Point *p0, g3Point *p1) {
  ...
  for (i = 0; i < 2; i++, vertp++) {
    ...
    if (OpenGL_state.cur_light_state != LS_NONE) {
      if (OpenGL_state.cur_light_state == LS_FLAT_GOURAUD) {
        colorp->r = fr;
        colorp->g = fg, colorp->b = fb;
        colorp->a = alpha;
      } else {
        // Do lighting based on intesity (MONO) or colored (RGB)
        if (OpenGL_state.cur_color_model == CM_MONO) {
          colorp->r = pnt->p3_uvl.l;
          colorp->g = pnt->p3_uvl.l;
          colorp->b = pnt->p3_uvl.l;
          colorp->a = alpha;
        } else {
          colorp->r = pnt->p3_uvl.r;
          colorp->g = pnt->p3_uvl.g;
          colorp->b = pnt->p3_uvl.r;
          colorp->a = alpha;
        }
      }
    } else {
