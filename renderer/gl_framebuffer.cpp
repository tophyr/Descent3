...
static const float framebuffer_buffer[] = {-1.0f, -3.0f, 0.0f, 0.0f, -1.0f, 1.0f, 0.0f, 2.0f, 3.0f, 1.0f, 2.0f, 2.0f};
...
void Framebuffer::Update(int width, int height, bool msaa) {
  ...
  if (msaa) {
    ...
    // When multisampling is used, the sub framebuffer is a non-multisampled framebuffer for the main framebuffer to
    // resolve to.
    if (m_subname == 0) {
    ...
    glTexImage2DMultisample(GL_TEXTURE_2D_MULTISAMPLE, SAMPLE_COUNT, GL_RGBA8, OpenGL_state.screen_width,
                            OpenGL_state.screen_height, GL_FALSE);

    glBindTexture(GL_TEXTURE_2D_MULTISAMPLE, m_depthname);
    glTexImage2DMultisample(GL_TEXTURE_2D_MULTISAMPLE, SAMPLE_COUNT, GL_DEPTH_COMPONENT32F, OpenGL_state.screen_width,
                            OpenGL_state.screen_height, GL_FALSE);

    glBindTexture(GL_TEXTURE_2D, m_subcolorname);
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB8, OpenGL_state.screen_width, OpenGL_state.screen_height, 0, GL_RGB,
    ...
    glTexImage2D(GL_TEXTURE_2D, 0, GL_DEPTH_COMPONENT32F, OpenGL_state.screen_width, OpenGL_state.screen_height, 0,
