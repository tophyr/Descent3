...
static void DelayUS(uint64_t us) { std::this_thread::sleep_for(std::chrono::microseconds(us)); }

static void timer_stop(void) { timer_started = 0; }

static void timer_start(void) { timer_started = 1; }
...
static int display_video_handler(unsigned char major, unsigned char minor, unsigned char *data, int len,
  ...
  if (ShowFrameCallback == NULL) {
    ...
    if (g_truecolor)
      (*ShowFrameCallback)((uint8_t *)g_vBackBuf1, g_width * 2, g_height, 0, 0, g_width, g_height, g_destX, g_destY,
                           true);
    else
...
void MVE_sfCallbacks(mve_cb_ShowFrame *func) { ShowFrameCallback = func; }

void MVE_palCallbacks(mve_cb_SetPalette *func) { SetPaletteCallback = func; }

void MVE_ioCallbacks(mve_cb_FileRead *func) {}

void MVE_memCallbacks(mve_cb_Malloc *alloc, mve_cb_Free *free) {}

void MVE_ReleaseMem() { hackBuf1 = hackBuf2 = NULL; }
