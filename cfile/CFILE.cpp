...
// NULL-terminated list of 								file extensions, & the dir will
// only be searched for files with a matching extension Returns:
// true if directory added, else false
...
double cf_ReadDouble(CFILE *cfp) {
  ...
  uint64_t heh = b[0] | (b[1] << 8) | (b[2] << 16) | (b[3] << 24) | ((uint64_t)b[4] << 32) | ((uint64_t)b[5] << 40) |
