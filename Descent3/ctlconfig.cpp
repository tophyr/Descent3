    ...
    if (slot != -1) {
      if (m_selection_status == -1) {
        ...
        if (!m_weapon_choose.IsCreated()) {
          ...
          m_weapon_choose.Create(m_menu, &UITextItem(MONITOR9_NEWUI_FONT, TXT_WPNCFGHELP2, GR_RGB(240, 240, 0)),
                                 WPNSEL_PRIMARY_X, WPNSEL_PRIMARY_Y - 48);
          if (focused_gadget)
...
void wpnsel_cfg_screen::realize() {
  ...
  m_reset_btn.Create(m_menu, UID_RESETDEFAULTS, TXT_RESETTODEFAULT, KEYCFG_EXTRAS_X + m_sheet->X(),
                     KEYCFG_EXTRAS_Y + m_sheet->Y(), NEWUI_BTNF_LONG);
  m_primary_title.Create(m_menu, &UITextItem(MONITOR9_NEWUI_FONT, TXT_PRIMARY), WPNSEL_PRIMARY_X,
                         WPNSEL_PRIMARY_Y - 28);
  m_secondary_title.Create(m_menu, &UITextItem(MONITOR9_NEWUI_FONT, TXT_SECONDARY), WPNSEL_SECONDARY_X,
                           WPNSEL_SECONDARY_Y - 28);
  m_primary_low.Create(m_menu, &UITextItem(MONITOR9_NEWUI_FONT, TXT_LOWPRIORITY), WPNSEL_PRIMARY_X,
                       WPNSEL_SECONDARY_Y - 11);
  m_secondary_low.Create(m_menu, &UITextItem(MONITOR9_NEWUI_FONT, TXT_LOWPRIORITY), WPNSEL_SECONDARY_X,
                         WPNSEL_SECONDARY_Y - 11);
  for (i = 0; i < MAX_PRIMARY_WEAPONS; i++) {
    int wpnidx;
    char str[64];
  ...
  m_primary_high.Create(m_menu, &UITextItem(MONITOR9_NEWUI_FONT, TXT_HIGHPRIORITY), WPNSEL_PRIMARY_X,
                        WPNSEL_PRIMARY_Y + i * 12 + 2);
  m_secondary_high.Create(m_menu, &UITextItem(MONITOR9_NEWUI_FONT, TXT_HIGHPRIORITY), WPNSEL_SECONDARY_X,
                          WPNSEL_SECONDARY_Y + i * 12 + 2);
