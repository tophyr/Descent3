...
void GenerateVertex(RendVertex &vert, int x, int y, int z, int basex, int basez, terrain_segment &basecell,
                    bool altnormal) {
  ...
  int tile = rotation >> 4;
  switch (texmode) {
  case 0: // This seems wrong, but the coordinate system of the new renderer pipeline is different (depth down -Z as per
          // OpenGL) and this has the right results.
...
void MeshTerrainCell(int x, int z) {
  ...
  int xstart = x * OCCLUSION_SIZE;
  int xend = xstart + OCCLUSION_SIZE;
  int zstart = z * OCCLUSION_SIZE;
  int zend = zstart + OCCLUSION_SIZE;
  int offset = 0;
...
void RenderTerrain(ubyte from_mine, int left, int top, int right, int bot) {
  ...
  rend_SetLighting(LS_NONE);
  rend_SetWrapType(
      WT_WRAP); // Should this be clamp? Requires smarter logic for the UV calculations to handle discontinuities.
