...
//[ISB] temp: Should stick this in Room or a separate render-related struct later down the line (dynamic room limit at
// some point?) These are the meshes of all normal room geometry.
...
void UpdateRoomMesh(int roomnum) {
  Room_meshes[roomnum]
      .Destroy(); // this kills the old mesh even if the room isn't used, since the stale mesh isn't used either.
...
void RenderFogFaces(room *rp) {
  ...
  for (int i = 0; i < Num_fog_faces_to_render; i++) {
    ...
    for (vn = 0; vn < fp->num_verts; vn++) {
      ...
      float mag = 1.f; //[ISB] Initialize this so we don't get junk data, but I can't tell if we're supposed to get here
                       // with Room_fog_plane_check==-1
