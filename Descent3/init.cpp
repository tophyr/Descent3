...
bool MercInstalled() { return Merc_IsInstalled; }
...
void InitIOSystems(bool editor) {
  ...
  if (!portableexists && !Portable) {
    if (!ddio_DirExists(userdir)) {
      ...
      if (ddio_FindFileStart("*.plt", temp)) {
        ddio_FindFileClose();
        int value = OutrageMessageBox(
            MBOX_YESNO, "A .plt file has been found in the working directory. \nWould you like to convert this install "
                        "to a portable install?\nNote that this will not work if installed in Program Files!");
        if (value == 7) {
  ...
  if (cfexist("BP_hud.inf") && cfexist("BlackPyroCockpit.OOF") && cfexist("Blackpyrocockpit.inf") &&
