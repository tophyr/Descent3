...
static int generate_mission_listbox_worker(newuiListBox *lb, int n_maxfiles, char **filelist, const char *pathname,
                                           const char *wildcard) {
  int c = 0;
  char fullpath[_MAX_PATH];
  char filename[PSPATHNAME_LEN];
...
inline int generate_mission_listbox(newuiListBox *lb, int n_maxfiles, char **filelist, const char *pathname,
                                    const char *wildcard) {
  std::future<int> c =
      std::async(std::launch::async, generate_mission_listbox_worker, lb, n_maxfiles, filelist, pathname, wildcard);
