...
inline int tga_read_int() {
  ...
  int i = Tga_file_data[Fake_pos] | (Tga_file_data[Fake_pos + 1] << 8) | (Tga_file_data[Fake_pos + 2] << 16) |
...
inline short tga_read_short() {
  ...
  short i = Tga_file_data[Fake_pos] | (Tga_file_data[Fake_pos + 1] << 8);
  ...
  for (m = 0; m < num_mips; m++) {
    ...
    while (count != total) {
      if (count >= total && !mipwarning) {
        // This condition doesn't abort, but it will stop reading data. This is so that the correct amount of data is
        // read.
