...
void ddio_MouseSetCallbackFn(DDIO_ShouldCaptureMouse_fp fn) { DDIO_ShouldCaptureMouse = fn; }
...
bool InitNewMouse() {
  ...
  if (!rawInputOpened) {
    ...
    rawInputDevice.dwFlags = RIDEV_INPUTSINK; //[ISB] I'm not happy having to do inputsink but for now it's needed to
                                              // avoid losing mouse inputs
    ...
    DDIO_mouse_state.nbtns =
        N_DIMSEBTNS + 3; // always have a mousewheel. [ISB] disgusting hack: Can't change mousewheel bindings for old
                         // pilots, so make button 5 after the two mouse wheel buttons.
