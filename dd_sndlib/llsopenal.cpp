...
int llsOpenAL::InitSoundLib(char mixer_type, oeApplication *sos, unsigned char max_sounds_played) {
  ...
  if (EffectsSupported) {
    ...
    if (!dalGenAuxiliaryEffectSlots || !dalDeleteAuxiliaryEffectSlots || !dalGenEffects || !dalDeleteEffects ||
        !dalEffecti || !dalEffectf || !dalAuxiliaryEffectSloti) {
...
bool llsOpenAL::LockSound(int sound_uid) { return false; }

bool llsOpenAL::UnlockSound(int sound_uid) { return false; }
...
char llsOpenAL::GetSoundQuality(void) { return Quality; }

bool llsOpenAL::SetSoundMixer(char mixer_type) { return false; }

char llsOpenAL::GetSoundMixer(void) { return SOUND_MIXER_DS3D_16; }
...
int llsOpenAL::PlaySound2d(play_information *play_info, int sound_index, float volume, float pan, bool f_looped) {
  ...
  if (ALErrorCheck("Setting 2D sound source properties."))
    return -1;
  ...
  if (ALErrorCheck("Binding 2D sound source buffer"))
    return -1;
  alSourcei(SoundEntries[sound_uid].handle, AL_BUFFER, SoundEntries[sound_uid].bufferHandle);
  if (ALErrorCheck("Setting 2D sound source buffer"))
    return -1;
  ...
  // PutLog(LogLevel::Info, "Starting 2D sound %s with uid %d (slot %d)",
  // pSoundFiles[pSounds[sound_index].sample_index].name, SoundEntries[sound_uid].soundUID, sound_uid);
...
int llsOpenAL::PlayStream(play_information *play_info) {
  ...
  if (SoundEntries[sound_uid].streamFormat == -1)
    return -1;

  // PutLog(LogLevel::Info, "Queueing initial data");
  alBufferData(SoundEntries[sound_uid].bufferHandles[0], SoundEntries[sound_uid].streamFormat, play_info->m_stream_data,
               play_info->m_stream_bufsize, 22050);
  ...
  NumSoundsPlaying++;
  // PutLog(LogLevel::Info, "Starting stream with uid %d (slot %d). Format is %d. Buffer size is %d. Stream size is
  // %d.", SoundEntries[sound_uid].soundUID, sound_uid, play_info->m_stream_format, play_info->m_stream_bufsize,
  // play_info->m_stream_size);
...
void llsOpenAL::SetListener(pos_state *cur_pos) {
  ...
  // Update listener
  ALfloat buf[6];
  buf[0] = -ListenerOrient.fvec.x;
  buf[1] = ListenerOrient.fvec.y;
  buf[2] = ListenerOrient.fvec.z;
  buf[3] = -ListenerOrient.uvec.x;
  buf[4] = ListenerOrient.uvec.y;
  buf[5] = ListenerOrient.uvec.z;
...
int llsOpenAL::PlaySound3d(play_information *play_info, int sound_index, pos_state *cur_pos, float master_volume,
  ...
  if (looped)
    alSourcei(SoundEntries[sound_uid].handle, AL_LOOPING, AL_TRUE);
  if (ALErrorCheck("Setting 3D sound source properties."))
    return -1;
  ...
  alSourcei(SoundEntries[sound_uid].handle, AL_BUFFER, 0);
  BindBufferData(SoundEntries[sound_uid].bufferHandle, sound_index, looped);
  if (ALErrorCheck("Binding 3D sound source buffer."))
    return -1;
  alSourcei(SoundEntries[sound_uid].handle, AL_BUFFER, SoundEntries[sound_uid].bufferHandle);
  if (ALErrorCheck("Setting 3D sound source buffer."))
    return -1;
  ...
  SoundEntries[sound_uid].soundNum = sound_index;
  SoundEntries[sound_uid].soundUID = NextUID * 256 + sound_uid;
  // PutLog(LogLevel::Info, "Starting 3D sound %s with uid %d (slot %d)",
  // pSoundFiles[pSounds[sound_index].sample_index].name, SoundEntries[sound_uid].soundUID, sound_uid);
...
void llsOpenAL::AdjustSound(int sound_uid, float f_volume, float f_pan, unsigned short frequency) {
  ...
  if (!Initalized)
    return;
  if (!SoundEntries || id < 0 || id >= NumSoundChannels || SoundEntries[id].soundUID != sound_uid)
    return;
...
void llsOpenAL::AdjustSound(int sound_uid, pos_state *cur_pos, float adjusted_volume, float reverb) {
  ...
  if (!SoundEntries || id < 0 || id >= NumSoundChannels || SoundEntries[id].soundUID != sound_uid ||
      isnan<float>(cur_pos->position->x))
    return;
  ...
  if (ALErrorCheck("Adjusting sound position."))
    mprintf((0, "\t(%f %f %f) (%f %f %f)", -cur_pos->velocity->x, cur_pos->velocity->y, cur_pos->velocity->z,
             -cur_pos->position->x, cur_pos->position->y, cur_pos->position->z));
...
void llsOpenAL::StopSound(int sound_uid, unsigned char f_immediately) {
  ...
  if (SoundEntries[id].playing) {
    if (f_immediately == 1) {
      ...
      // PutLog(LogLevel::Info, "External code trying to stop sound %d (%s). Elimination mode %d.", sound_uid,
      // pSoundFiles[pSounds[SoundEntries[id].soundNum].sample_index].name, f_immediately);
    } else if (islooping) {
...
bool llsOpenAL::CheckAndForceSoundDataAlloc(int sound_file_index) {
  ...
  if ((SoundFiles[sound_file_index_00].sample_16bit == NULL) && (SoundFiles[sound_file_index_00].sample_8bit == NULL)) {
    ...
    cVar1 = SoundLoadWaveFile(SoundFiles[sound_file_index_00].name, Sounds[sound_file_index].import_volume,
                              sound_file_index_00, Quality == SQT_HIGH, true, NULL);
    return cVar1 != '\0';
  }
  return true;
}
...
void llsOpenAL::InitSource3D(uint32_t handle, sound_info *soundInfo, pos_state *posInfo, float volume) {
  ...
  // PutLog(LogLevel::Info, "Starting 3d sound at %f %f %f", posInfo->position->x, posInfo->position->y,
  // posInfo->position->z);
...
void llsOpenAL::BindBufferData(uint32_t handle, int sound_index, bool looped) {
  ...
  void *ptr = Quality == SQT_HIGH ? (ALvoid *)SoundFiles[Sounds[sound_index].sample_index].sample_16bit
                                  : (ALvoid *)SoundFiles[Sounds[sound_index].sample_index].sample_8bit;
  int len = bpp * SoundFiles[Sounds[sound_index].sample_index].np_sample_length;

  // PutLog(LogLevel::Info, "Buffering sound id %d (%s)", sound_index,
  // pSoundFiles[pSounds[sound_index].sample_index].name);
  if (!ptr) {
...
void llsOpenAL::SoundCleanup(int soundID) {
  ...
  // PutLog(LogLevel::Info, "Sound %d (%s) has been stopped", SoundEntries[i].soundUID,
  // pSoundFiles[pSounds[SoundEntries[i].soundNum].sample_index].name);
...
void llsOpenAL::ServiceStream(int soundID) {
  ...
  if (numProcessedBuffers > 0) {
    ...
    // PutLog(LogLevel::Info, "%d buffers dequeued. %d currently queued. [%d %d %d]", numProcessedBuffers,
    // numQueuedBuffers, SoundEntries[soundID].bufferQueue[0], SoundEntries[soundID].bufferQueue[1],
    // SoundEntries[soundID].bufferQueue[2]);
  }
  ...
  if (numQueuedBuffers < NUM_STREAMING_BUFFERS) // queue up a new one
  {
    ...
    if (newSlot == -1) {
      // eh, this needs to be reconsidered some. Can be race conditions here since the song is still playing before
      // stuff is updated.
      return;
    }

    // Get available data and queue it
    data = SoundEntries[soundID].info->m_stream_cback(SoundEntries[soundID].info->user_data,
                                                      SoundEntries[soundID].info->m_stream_handle, &size);
