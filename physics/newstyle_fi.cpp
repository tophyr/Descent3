...
bool PolyCollideObject(object *obj) {
  ...
  } else // [ISB] Huge confusion here about whether or not this should ever be approached with a objnum < 0. Object 0,
         // the local player, is treated differently as a result.
  {
#ifndef NED_PHYSICS
    if ((Game_mode & GM_MULTI) && !(Netgame.flags & NF_USE_ACC_WEAP) && Objects[-fvi_moveobj].type == OBJ_WEAPON &&
        obj->type == OBJ_PLAYER)
