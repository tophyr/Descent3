...
void TakeOverRoom(int newteam, int oldteam, int roomnum, int victor) {
  ...
  sprintf(buffer, TXT_TAKEOVER, (victor != -1) ? dPlayers[victor].callsign : TXT_NONAME,
...
void OnPLRInterval(void) {
  ...
  if (Player_who_took_over_last_base != -1) {
    ...
    char message[384];
    sprintf(message, "%s successfully took over %s team's last base!",
...
void OnPrintScores(int level) {
  ...
  for (i = 0; i < pcount; i++) {
    ...
    if ((pr) && (pr->state != STATE_EMPTY)) {
      ...
      sprintf(name, "%s%s: %.8s", (pr->state == STATE_DISCONNECTED) ? "*" : "", pr->callsign,
